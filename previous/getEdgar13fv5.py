...
class Cik:
    ...
    def process_cik(self, cik):
	    ...
	    # get the filing as text      
        txt_file = requests.get(txt_link).content
        
        # file contains XML tags but is not well formed xml    
	   ...
	   # parse the primary doc
        primary_doc_parser = etree.fromstring(primary_doc)               
        # manager name
        mgr_name = primary_doc_parser.find(self.filingManagerName_tag).text
        ...
        for infoField in info_table_parser.findall(self.infoTable_tag):          
            ...
            commaList.append(infoField.find(self.cusip_tag))
            commaList.append(infoField.find(self.nameOfIssuer_tag))
            commaList.append(infoField.find(self.value_tag))
            commaList.append(infoField.find(self.sshPrnamt_tag))        
            commaList.append(infoField.find(self.titleOfClass_tag))
...
if __name__ == '__main__':
    ...
    # 0001425999
    print len(ticker.process_cik('0001425999'))
